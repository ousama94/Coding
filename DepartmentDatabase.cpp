...
{
	Member *temp = new Member();	//This constructs a new node every time it's called, pointer type of class member which points to the address of a new node
...
void DeptList::print_list()         //Print list function, the pointer initially points to the address of the latest entry, and will keep pointing to the next node until it reaches the tail
{
	Member *peek;
